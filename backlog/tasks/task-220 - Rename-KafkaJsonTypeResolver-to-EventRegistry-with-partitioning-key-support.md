...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 `KafkaJsonTypeResolver` is renamed to `EventRegistry` in `prefab-core`
- [x] #2 `EventRegistry` gains `registerType(topic, type, keyExtractor)` overload and `keyFor(event)` method
- [x] #3 `KafkaEventTypeRegistrarWriter` generates key extractor lambda registration when `@PartitioningKey` is present
- [x] #4 `KafkaProducerWriter` uses `eventRegistry.keyFor(event).orElse(null)` instead of inline key extraction
- [x] #5 All references updated: `DynamicDeserializer`, `KafkaTopicResolver`, `StreamsConfiguration`, tests
<!-- AC:END -->
