...
Note: @Example has no effect when placed directly on aggregate fields. The annotation is consumed during the processing of constructor parameters and event record components.
...
- [ ] #10 The annotation processor emits a compile-time warning if an @Example value cannot be parsed as the target field type (e.g. @Example("not-a-number") on an int field), helping catch annotation mistakes early.
<!-- AC:END -->
