...
public class SingleValuedNestedEventOuterToGenericRecordConverter implements Converter<SingleValuedNestedEvent.Outer, GenericRecord> {
    ...
    public GenericRecord convert(SingleValuedNestedEvent.Outer event) {
        ...
        genericRecord.put("inner", event.inner() != null ? SchemaSupport.isRecordSchema(schema.getField("inner").schema()) ? SchemaSupport.singleValueRecord(schema.getField("inner").schema(), "value", event.inner().value()) : event.inner().value() : null);
