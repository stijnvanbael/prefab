...
`PublishedEventsExtension` (see [7.12 Unit Testing Domain Events](#712-unit-testing-domain-events)).
...
between tests, and in particular that unit tests do not poison the static `DomainEventPublisher`
instance that integration tests rely on.
...
— the next test (or Spring context startup) gets a fresh slate.
