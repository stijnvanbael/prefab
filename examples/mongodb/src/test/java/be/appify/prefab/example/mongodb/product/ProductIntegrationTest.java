...
class ProductIntegrationTest {
    ...
    void createAndRetrieveProduct() throws Exception {
        var categoryId = categories.createCategory("Gadgets").id();
        var productId = products.createProduct("Widget", "A useful widget", BigDecimal.valueOf(9.99), "USD", categoryId).id();

        var product = products.getProductById(productId).response();
    ...
    void listProductsWithFilter() throws Exception {
        var categoryId = categories.createCategory("Tools").id();
        ...
        var page = products.findProducts(null, "Alpha").response();
    ...
    void updateProduct() throws Exception {
        var categoryId = categories.createCategory("Widgets").id();
        var productId = products.createProduct("Old Name", "Old Desc", BigDecimal.ONE, "USD", categoryId).id();

        products.update(productId, "New Name", "New Desc");

        var updated = products.getProductById(productId).response();
    ...
    void deleteProduct() throws Exception {
        var categoryId = categories.createCategory("Disposable").id();
        var productId = products.createProduct("Temp", "Temporary product", BigDecimal.ONE, "USD", categoryId).id();

        products.deleteProduct(productId);

        var page = products.findProducts(null, null).response();
