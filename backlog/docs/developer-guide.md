...
| `prefab.outbox.poll-interval-ms` | `1000` | How often the relay polls the outbox (ms) |
| `prefab.outbox.batch-size` | `100` | Max outbox entries processed per relay cycle |
...
2. After `repository.save(aggregate)` (JDBC or MongoDB), the buffer is drained and each event is written
   to the `prefab_outbox` table/collection **in the same transaction**.
3. The `OutboxRelayService` polls the outbox on a schedule, publishes each event via the Spring
   `ApplicationEventPublisher` (which triggers Kafka/SNS/Pub-Sub producers), and then deletes the entry.
...
**Schema (PostgreSQL):** the `prefab_outbox` table is auto-generated by the Prefab annotation processor
as part of the standard Flyway migration. No manual DDL is required.

**Schema (MongoDB):** the `prefab_outbox` collection is created automatically on first insert.
