...
public class KafkaPrefabStreams implements PrefabStreams {
    ...
    @Override
    public <VS> Store<VS> createStore(Class<VS> type, StreamProcessorContext<?> context) {
        var name = toKebabCase(type.getName());
        streamsBuilder.addStateStore(Stores.keyValueStoreBuilder(
                Stores.persistentKeyValueStore(name),
                Serdes.String(),
                new SerdeAdapter<>(serializer, deserializer)));
        return new KafkaPrefabStoreAdapter<>(name, (KafkaPrefabProcessorContext<?>) context);
    }
