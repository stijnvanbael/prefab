...
public class CreatePlugin implements PrefabPlugin {
    ...
    @Override
    public void writeTestFixture(ClassManifest manifest, TypeSpec.Builder builder) {
        createConstructorOf(manifest).ifPresent(createConstructor ->
                testFixtureWriter.createMethods(manifest, createConstructor).forEach(builder::addMethod));
    }
