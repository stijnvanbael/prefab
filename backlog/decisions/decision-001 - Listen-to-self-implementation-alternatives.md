...
### Alternative B — `@EventSourced` on the aggregate class
...
Non-`@EventSourced` aggregates like `ChannelSummary` are completely unaffected — they continue to generate
...
`@EventSourced` makes the contract visible and uniform at the class level. The processor can derive all code
generation from a single declaration. The `@Update void` ambiguity disappears entirely — on an `@EventSourced`
...
- **Fully backward compatible**: only aggregates explicitly annotated with `@EventSourced` change behaviour.
  All existing aggregates are unaffected.
- One new source-retained annotation is needed: `@EventSourced` on the aggregate class.
- `@Create` on an `@EventSourced` aggregate must be a **static method returning the event type**. The constructor
  form is a compile error on `@EventSourced` aggregates (enforced by the processor).
- `@Update` `void` methods on `@EventSourced` aggregates are treated as publish-only; the processor omits the
  repository save and generates `202 Accepted`.
- `@EventHandler` methods on `@EventSourced` aggregates automatically generate broker consumers with a
...
- A new `@EventId` annotation is required on event fields. The processor enforces its presence on every event
  type consumed by an `@EventSourced` aggregate and uses it as the sole deduplication key.
- Documentation and integration tests must cover: `@EventSourced` create, update, replay with deduplication, and
