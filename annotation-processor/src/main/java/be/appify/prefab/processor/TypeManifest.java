...
public class TypeManifest {
    ...
    private TypeManifest(TypeMirror typeMirror, ProcessingEnvironment processingEnvironment) {
        ...
        this.unresolved = typeMirror.getKind() == TypeKind.ERROR;
    ...
    public static TypeManifest of(TypeMirror typeMirror, ProcessingEnvironment processingEnvironment) {
        if (containsUnresolvedType(typeMirror)) {
            // Unresolved type (possibly nested in type arguments) generated by another processor in the same round.
            // Do not cache — it will be re-resolved in a later round.
    ...
    /**
     * Returns {@code true} if this type could not be resolved to a real, compiled type at the time this manifest
     * was created — i.e. its {@link TypeMirror} had kind {@code ERROR}. This typically happens when a type is
     * generated by another annotation-processing plugin (e.g. an {@code @Avsc}-derived record) that has not yet
     * been written and compiled in an earlier round. Unresolved types should never be treated as
     * {@linkplain #isStandardType() standard types}; callers that depend on knowing a field's real shape (nested
     * record vs. primitive/JDK type) should defer processing until the type resolves in a later round.
     *
     * @return {@code true} if the type is unresolved, {@code false} otherwise
     */
