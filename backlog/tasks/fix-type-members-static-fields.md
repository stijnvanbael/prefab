...
- [ ] `TypeMembers.backingFields()` ignores `static` fields on non-record types
- [ ] `TypeMembers.recordFieldsWithComponentAnnotations()` ignores `static` fields on record types
- [ ] A record with static constant fields of its own type generates a correct `Builder`
      (only instance components appear as builder parameters)
- [ ] Existing test suite passes
- [ ] New unit test in `TypeMembersTest` (or equivalent) covers a record with static constants
