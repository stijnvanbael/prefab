...
class TypeMembers {
     ...
     * to a real, compiled type.
     * <p>
     * During annotation processing, a permitted subtype that is still being generated by another
     * plugin (e.g. an AVSC-derived record) appears as an unresolved {@link TypeKind#ERROR} type
     * until a later round. {@link #permittedSubtypes()} silently filters those out, so callers that
     * need the complete, closed set of subtypes must check this first and defer otherwise.
     */
    boolean permittedSubtypesResolved() {
        return element == null || element.getPermittedSubclasses().stream()
                .noneMatch(type -> type.getKind() == TypeKind.ERROR);
    }
