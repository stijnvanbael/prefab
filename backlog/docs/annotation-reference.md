...
- On `@Create` static factory method: must have `void` return type and call `PublishesEvents.publishEvent(event)`
  internally; the generated endpoint returns `202 Accepted`. The event is routed through the Spring application event
  bus to the generated Kafka (or Pub/Sub, SNS) producer.
- On `@Update void` method: method must call `PublishesEvents.publishEvent(event)` internally; generates `202 Accepted`;
...
| Value      | SQL shape       | Mongo regex | Description                         |
|------------|-----------------|-------------|-------------------------------------|
| `PREFIX`   | `LIKE 'term%'`  | `^term`     | Value must **start with** the term. |
| `CONTAINS` | `LIKE '%term%'` | `term`      | Value must **contain** the term.    |

#### `MatchStrategy`

| Value         | JDBC effect                                                                            | MongoDB effect                                                         |
|---------------|----------------------------------------------------------------------------------------|------------------------------------------------------------------------|
...
```java
// Prefix, case-insensitive (default)
@Autocomplete
String name;

// Contains, exact case
@Autocomplete(scanMode = ScanMode.CONTAINS, matchStrategy = MatchStrategy.EXACT)
String sku;

// Custom path, fuzzy (PostgreSQL only)
...
The old `boolean ignoreCase` attribute has been removed. Replace it as follows:

| Old                                 | New equivalent                                                                           |
|-------------------------------------|------------------------------------------------------------------------------------------|
| `@Autocomplete`                     | `@Autocomplete` *(default is now `PREFIX` + `IGNORE_CASE`)*                              |
| `@Autocomplete(ignoreCase = true)`  | `@Autocomplete(scanMode = ScanMode.CONTAINS, matchStrategy = MatchStrategy.IGNORE_CASE)` |
| `@Autocomplete(ignoreCase = false)` | `@Autocomplete(matchStrategy = MatchStrategy.EXACT)`                                     |
...
- `{Type}KafkaEventTypeRegistrar` — registers the event type (and its partitioning key extractor, if any) with
  `EventRegistry`
...
- When `@DbMigration` is enabled, the value of `type()` is emitted **verbatim** in the generated `CREATE TABLE` or
  `ALTER TABLE` statement.
- When `converter()` is specified (non-void), the class is instantiated and registered automatically as a
  `JdbcCustomConversions` contributor. The converter must have a public no-argument constructor.
